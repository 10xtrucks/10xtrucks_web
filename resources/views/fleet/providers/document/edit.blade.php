...
<div class="content-area py-1">
    <div class="container-fluid">
        ...
        <div class="box box-block bg-white">
            <h5 class="mb-1">@lang('fleet_dispatcher.provider_name'): {{ $Document->provider->first_name }} {{ $Document->provider->last_name }}</h5>
            ...
            <div class="row">
                <div class="col-xs-6">
                    <form action="{{ route('fleet.provider.document.update', [$Document->provider->id, $Document->id]) }}" method="POST">
                        {{ csrf_field() }}
                        {{ method_field('PATCH') }}
                        <button class="btn btn-block btn-primary" type="submit">@lang('fleet_dispatcher.approve')</button>
                    </form>
                </div>
                ...
                <div class="col-xs-6">
                    <form action="{{ route('fleet.provider.document.destroy', [$Document->provider->id, $Document->id]) }}" method="POST">
                        {{ csrf_field() }}
                        {{ method_field('DELETE') }}
                        <button class="btn btn-block btn-danger" type="submit">@lang('fleet_dispatcher.delete')</button>
                    </form>
                </div>
            </div>
        </div>
