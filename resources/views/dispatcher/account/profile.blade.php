    	...
    	<div class="box box-block bg-white">
            ...
            <form class="form-horizontal" action="{{route('dispatcher.profile.update')}}" method="POST" enctype="multipart/form-data" role="form">
				...
				<div class="form-group row">
					...
					<div class="col-xs-10">
						<input class="form-control" type="text" value="{{ Auth::guard('dispatcher')->user()->name }}" name="name" required id="name" placeholder=" @lang('fleet_dispatcher.name')">
					</div>
				</div>
				...
				<div class="form-group row">
					...
					<div class="col-xs-10">
						<input class="form-control" type="email" required name="email" value="{{ isset(Auth::guard('dispatcher')->user()->email) ? Auth::guard('dispatcher')->user()->email : '' }}" id="email" placeholder="@lang('fleet_dispatcher.email')">
					</div>
				</div>
				...
				<div class="form-group row">
					...
					<div class="col-xs-10">
						<input class="form-control" type="text" value="{{ Auth::guard('dispatcher')->user()->mobile }}" name="mobile" required id="mobile" placeholder=" @lang('fleet_dispatcher.mobile')">
					</div>
				</div>
