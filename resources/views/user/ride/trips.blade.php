    ...
    <div class="dash-content">
        ...
        <div class="row no-margin ride-detail">
            ...
            @if($trips->count() > 0)
                ...
                <table class="table table-condensed" style="border-collapse:collapse;">
                    ...
                    @foreach($trips as $trip)
                        ...
                        <tr data-toggle="collapse" data-target="#trip_{{$trip->id}}" class="accordion-toggle collapsed">
                            ...
                            <td>{{$trip->assigned_at->toDayDateTimeString()}}</td>
                            @if($trip->provider)
                            ...
                            @else
                                <td>-</td>
                            @endif
                             @if($trip->service_items)
                                <td>{{$trip->service_items}}</td>
                            @else
                                <td>-</td>
                            @endif
                        ...
                        <tr class="hiddenRow">
                            <td colspan="12">
                                <div class="accordian-body collapse row" id="trip_{{$trip->id}}">
                                    ...
                                    $static_map = "https://maps.googleapis.com/maps/api/staticmap?autoscale=1&size=600x450&maptype=terrain&format=png&visual_refresh=true&markers=icon:".$map_icon."%7C".$trip->s_latitude.",".$trip->s_longitude."&markers=icon:".$map_icon."%7C".$trip->d_latitude.",".$trip->d_longitude."&path=color:0x191919|weight:8|enc:".$trip->route_key."&key=".Setting::get('google_map_key'); ?>
                ...
                @else
                    <hr>
                    <p style="text-align: center;">No Couriers Available</p>
                @endif
            </div>
        </div>
