    ...
    render() {
        ...
        switch(this.state.listContent) {
            ...
            case 'dispatch-assign':
                ...
                break;
            case 'dispatch-bidding':
                listContent = <div className="col-md-4">
                        <DispatcherBiddingAssignList trip={this.state.trip} />
                    </div>;                    
                break;
...
class DispatcherBiddingAssignList extends React.Component {
    constructor(props) {
        super(props);
        this.state = {
            data: {
    ...
    componentDidMount() {
        $.get('/admin/dispatcher/bidding/providers', { 
            reqeust_id: this.props.trip.id,
        }, function(result) {
            console.log('Providers', result);
            if(result.hasOwnProperty('data')) {
                this.setState({
    ...
    render() {
        console.log('DispatcherBiddingAssignList - render', this.state.data);
        return (
            <div className="card">
                <div className="card-header text-uppercase"><b>Assign Provider</b></div>
                
                <DispatcherBiddingAssignListItem data={this.state.data.data} trip={this.props.trip} />
            </div>
        );
    }
}

class DispatcherBiddingAssignListItem extends React.Component {
    handleClick(provider) {
        // this.props.clicked(trip)
        console.log('Provider Clicked');
        window.assignProviderPopPicked(provider);
                        ...
                        <a className="text-black" href="#">
                            <div className="media">
                                <div className="media-body">
                                    ...
                                    <h6 className="media-heading">Phone: {provider.mobile}</h6>
                                    <h6 className="media-heading">Type: {provider.service.service_type.name}</h6>
                                    <h5 className="media-heading">Bidding Amount: {provider.providerbidding.bidding_amount}</h5>
                                </div>
                            </div>
                        </a>
                    </div>
