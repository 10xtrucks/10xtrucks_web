...
class AdminController extends Controller
{
    ...
    {
        $details = GeoFencing::get();

        return view('admin.geo_fencing.geo',compact('details'));
    }


    public function geo_fencing_store(Request $request)
    {
        //dd($request->all());
        try{
            if(!empty($request->service_range)){
            $ranges = explode('/',$request->service_range);
            foreach($ranges as $range)
            {
                $data['ranges'] = $range;
                GeoFencing::create($data);
            }

            return back()->with('flash_success','Settings Updated Successfully');
        }else{
            return back()->with('flash_error','No Region Found');
        }
        }
        catch(Exception $e)
        {
    ...
    public function settings_store(Request $request)
    {
        if(Setting::get('demo_mode', 0) == 1) {
            return back()->with('flash_error','Disabled for demo purposes! Please contact us at info@10xtrucks.com');
        }
        ...
        Setting::set('track_distance', $request->track_distance);
        Setting::set('help_content', $request->help);
        Setting::set('google_map_key', $request->google_map_key);
        Setting::set('twilio_accountsid', $request->twilio_accountsid);
        Setting::set('twilio_token', $request->twilio_token);
        Setting::set('twilio_mobile', $request->twilio_mobile);
        Setting::set('twilio_app_sid', $request->twilio_app_sid);
        Setting::set('twilio_secret', $request->twilio_secret);
        Setting::set('twilio_key', $request->twilio_key);
        Setting::set('default_country_code', $request->default_country_code);
        Setting::save();
    ...
    public function settings_payment_store(Request $request)
    {
        ...
        $this->validate($request, [
                ...
                'paystack_secret_key' => 'required_if:PAYSTACK,on|max:255',
                'paystack_publishable_key' => 'required_if:PAYSTACK,on|max:255',
        ...
        try {
            ...
            Setting::set('BOL', $request->has('BOL') ? 1 : 0 );
            Setting::set('PAYSTACK', $request->has('PAYSTACK') ? 1 : 0 );
            ...
            Setting::set('paystack_publishable_key', $request->paystack_publishable_key);
    ...
    {
        try {
            $str = file_get_contents('http://10xtrucks.com/help.json');
    ...
    public function privacy(){
        $page_cont = 'page_privacy';
        return view('admin.pages.static',compact('page_cont'))
    ...
    public function pages(Request $request){
        $this->validate($request, [
                'page' => 'required|in:page_privacy,terms',
                'content' => 'required',
            ]);
    ...
    public function statement($type = 'individual'){
        ...
        try{
            ...
            if($type == 'individual'){
                $page = 'Provider Delivery Statement';
            }elseif($type == 'today'){
