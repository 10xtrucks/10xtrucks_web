...
class DispatcherController extends Controller
{
     ...
     * @return \Illuminate\Http\Response
     */
    public function biddingProviderList(Request $request)
    {
        $ProviderIds = ProviderBidding::where('user_request_id', $request->reqeust_id)->pluck('provider_id')->toArray();
        $Providers = Provider::with('providerbidding', 'service', 'service.service_type')->whereIn('id', $ProviderIds)->paginate(10);
        return $Providers;
    }
    ...
    public function assign($request_id, $provider_id)
    {
        try {
            ...
            $providerBidding = ProviderBidding::where('user_request_id', $request_id)->where('provider_id', $provider_id)->first();

            if($Request->schedule_at != ""){

                $beforeschedule_time = strtotime($Request->schedule_at."- 1 hour");
                $afterschedule_time = strtotime($Request->schedule_at."+ 1 hour");
                ...
                if($CheckScheduling > 0 ){
                    return redirect()->route('admin.dispatcher.index')->with('flash_error', 'If the ride is already scheduled then we cannot schedule/request another ride for the after 1 hour or before 1 hour');
                }
                RequestFilter::where('request_id',$Request->id)->where('provider_id',Auth::user()->id)->update(['status' => 2]);

                $Request->status = "SCHEDULED";
                $Request->provider_id = $Provider->id;
                $Request->current_provider_id = $Provider->id;
                if ($providerBidding) {
                    $Request->bidding_amount = $providerBidding->bidding_amount;
                }
                $Request->save();
            }else{
                $Request->provider_id = $Provider->id;
            ...
            ProviderBidding::where('user_request_id', $request_id)->delete();
    ...
    public function store(Request $request) {
        ...
        try{
            ...
            $details = "https://maps.googleapis.com/maps/api/directions/json?origin=".$request->s_latitude.",".$request->s_longitude."&destination=".$request->d_latitude.",".$request->d_longitude."&mode=driving&key=".Setting::get('google_map_key');
